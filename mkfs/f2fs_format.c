...
static int f2fs_write_default_quota(int qtype, unsigned int blkaddr)
{
	...
	ddqheader.dqh_magic = htole32(file_magics[qtype]);
	/* only support QF_VFSV1 */
	ddqheader.dqh_version = htole32(1);

	memcpy(filebuf, &ddqheader, sizeof(ddqheader));

	/* Fill Initial quota file content */
	ddqinfo.dqi_bgrace = htole32(MAX_DQ_TIME);
	ddqinfo.dqi_igrace = htole32(MAX_IQ_TIME);
	ddqinfo.dqi_flags = htole32(0);
	ddqinfo.dqi_blocks = htole32(QT_TREEOFF + 5);
	ddqinfo.dqi_free_blk = htole32(0);
	ddqinfo.dqi_free_entry = htole32(5);
	...
	dqblk.dqb_id = htole32(0);
	dqblk.dqb_pad = htole32(0);
	dqblk.dqb_ihardlimit = htole64(0);
	dqblk.dqb_isoftlimit = htole64(0);
	dqblk.dqb_curinodes = htole64(1);
	dqblk.dqb_bhardlimit = htole64(0);
	dqblk.dqb_bsoftlimit = htole64(0);
	dqblk.dqb_curspace = htole64(4096);
	dqblk.dqb_btime = htole64(0);
	dqblk.dqb_itime = htole64(0);
